...
function googleError(){
    alert("ERROR: Error in loading google maps");
}
...
function setMarkers(location) {
    ...
    for (var i = 0; i < location.length; i++) {
        ...
        railwayTitle + '&format=json&callback=wikiCallback';
        ...
        new google.maps.event.addListener(location[i].locMarker, 'click',
            (function(railways, i) { return function() {
                railways.setAnimation(google.maps.Animation.BOUNCE);
                setTimeout(function() {
                    railways.setAnimation(null);
                }, 2400);
                infowindow.setContent(location[i].contentString);
                infowindow.open(map,this);
                map.setZoom(15);
                map.setCenter(railways.getPosition());
            };
        })(location[i].locMarker, i));

        /* info window call when clicked on railways menu item */
        var searchNav = $('#nav' + i);
        searchNav.click((function(railways, i) {
            return function() {
                railways.setAnimation(google.maps.Animation.BOUNCE);
                setTimeout(function() {
                    railways.setAnimation(null);
